...
/*select a value in the table*/
export async function exec_SelectElemsFromTableMultiplesArgsAND(db: Database, table: string, fields: string[], values: any[], require?: string): Promise<any[] | null> {
	let setText: string = '';
	if (fields.length !== values.length) return null;
	for (let i: number = 0; i < fields.length; i++) {
		setText = setText + (i > 0 ? ' AND ' : '') + `${fields[i]} = $${i + 1}`;
	}
	const selectElem: string = require ? require : '*';
	const query: string = `SELECT ${selectElem} FROM ${table} WHERE ${setText}`;
	// console.log('query')
