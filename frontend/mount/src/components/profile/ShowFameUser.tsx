...
function ShowFameUser({ userM }: Prop) {
    ...
    if (userM.fame_rating <= 3) {
        ...
        color =
            'bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded border border-blue-400';
        fame = `Low rated user (${userM.fame_rating})`;
    } else if (userM.fame_rating <= 8) {
        color =
            'bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded  border border-green-400';
        fame = `Popular user (${userM.fame_rating})`;
    } else if (userM.fame_rating <= 10) {
        color =
            'bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded 0 border border-yellow-300';
    ...
    return (
        <div className="grid grid-cols-4 pl-2 pt-2 pb-2 border-b border-gray-200">
            ...
            <div className="col-span-3">
                <span className={color}>{fame}</span>
            </div>
        </div>
    );
}
