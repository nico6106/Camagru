...
function ImageCarrousel({ pictures, visible, setVisible }: Prop) {
    ...
    <div className="relative h-56 overflow-hidden rounded-lg md:h-96">
		...
		{pictures.map((elem) => <ShowImgCarrousel picture={elem} visible={visible} key={elem} />)}
    </div>
    <div className="absolute z-30 flex space-x-3 -translate-x-1/2 bottom-5 left-1/2">
		{pictures.map((elem, index) => <PositionImgCarrousel nb={index} current={elem === visible} key={index} />)}
    </div>
    <button type="button" className="absolute top-0 left-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev onClick={handlePrevious}>
...
function ShowImgCarrousel({ picture, visible }: PropShowImg) {
	...
	return (
		<div className={classDiv} data-carousel-item>
            <img src={link} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="User image" onClick={onClickImg}/>
		</div>
	)
}
...
function PositionImgCarrousel({ nb, current }: PropButton) {
	return (<button type="button" className="w-3 h-3 rounded-full" aria-current={current} aria-label="Slide 1" data-carousel-slide-to={nb}></button>)
}
